...
import es.i12capea.marvel.common.ErrorMarvel
import es.i12capea.marvel.common.Event
import es.i12capea.marvel.domain.exceptions.PredicateNotSatisfiedException
import es.i12capea.marvel.domain.exceptions.RequestException
import es.i12capea.marvel.domain.exceptions.ResponseException
...
) : ViewModel(), CoroutineScope
{
    ...
    private val _error: MutableLiveData<Event<ErrorMarvel>> = MutableLiveData()
    val error : LiveData<Event<ErrorMarvel>> = _error
    ...
    fun handleError(cause: Throwable){
        when(cause){
            is RequestException -> {
                _error.postValue(Event(ErrorMarvel(1, "No se ha podido realizar la conexión.")))
            }
            is ResponseException -> {
                _error.postValue(Event(ErrorMarvel(2, "Error en la respuesta de servidor.")))
            }
            is PredicateNotSatisfiedException -> {
                _error.postValue(Event(ErrorMarvel(3, "No se ha cumplido los predicados.")))
            }
            else -> {
                _error.postValue(Event(ErrorMarvel(9999, "Error desconocido")))
            }
        }
    }
